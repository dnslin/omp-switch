...
- 自动备份。
- 跨文件可恢复事务。
- 应用设置仅保存 OMP 路径、主题、轻量 Provider/Model 选择和模型测试费用说明偏好；设置页提供后端受限的 Target、应用和日志目录入口、恢复默认设置、安全 OMP 切换、未保存确认、快捷键、通知和减少动画。
- Rust、前端和三平台手工验收。
...
4. 生成并验证全部临时文件。
5. 写入持久事务清单，记录原始 Hash、最终 Hash、逻辑分区（models/config）、解析后的真实目标路径和备份路径。
6. 依次原子替换目标文件。
7. 全部成功后删除事务清单；若清理失败，返回明确错误并保留清单，后续启动按最终 Hash 完成清理。


启动时发现未完成事务：

- 如果所有目标都匹配事务清单的最终 Hash，只完成事务清理；该判定优先于备份存在性检查，不因共享备份缺失回滚已完成的数据。
- 否则先保存当前现场副本，再预检同一事务的全部备份，并从这些备份恢复所有目标文件。
- 不允许只恢复其中一个文件。
- 如果任一目标恢复失败，立即停止后续恢复并将已写入目标回滚到恢复前现场；回滚失败时保留现场副本并进入只读人工处理，不报告整体恢复成功。
清单恢复时会重新解析当前 `target/models.yml` 与 `target/config.yml` 的真实路径，并与清单逐项精确比较；文件符号链接只要仍指向原目标即可自动整体恢复，备份仍按逻辑 `models/`、`config/` 分区保存。
